...
public class RequestSecurityAssistanceController {
    ...
    @FXML
    private TextArea txtAssistance;

    @FXML
    private void submitRequest() {

        if (txtAssistance.getText().trim().isEmpty()) {

            Alert alert = new Alert(Alert.AlertType.WARNING);
            alert.setTitle("Warning");
            alert.setHeaderText(null);
            alert.setContentText("Please enter your assistance request.");
            alert.showAndWait();
            return;
        }
    ...
    private void back() throws IOException {
        ...
        Stage stage = (Stage) txtAssistance.getScene().getWindow();
